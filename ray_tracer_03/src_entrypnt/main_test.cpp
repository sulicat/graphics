...
TEST( ColorTest, Add ){
  ...
  Color c = c1 + c2;
  EXPECT_FLOAT_EQ( c.r, 1.6 );
  EXPECT_FLOAT_EQ( c.g, 0.7 );
...
TEST( ColorTest, Subtract ){
  ...
  Color c2 = Color( 0.7, 0.1, 0.25 );
  Color c = c1 - c2;
  EXPECT_FLOAT_EQ( c.r, 0.2 );
  EXPECT_FLOAT_EQ( c.g, 0.5 );
  EXPECT_FLOAT_EQ( c.b, 0.5 );
}

TEST( ColorTest, MultiplyScalar ){
  Color c1 = Color( 0.2, 0.3, 0.4 );
  Color c = c1 * 2;
  EXPECT_FLOAT_EQ( c.r, 0.4 );
  EXPECT_FLOAT_EQ( c.g, 0.6 );
  EXPECT_FLOAT_EQ( c.b, 0.8 );
}
...
TEST( ColorTest, MultiplyColor ){
  ...
  Color c = c1 * c2;
  EXPECT_FLOAT_EQ( c.r, 0.9 );
  EXPECT_FLOAT_EQ( c.g, 0.2 );
  EXPECT_FLOAT_EQ( c.b, 0.04 );
}
